...
const Filter = () => {
    ...
    const onGetSensors = useCallback((site, status, identifier) => dispatch(action.getSensors(site, status, identifier)), [dispatch]);

    const [siteFilter, setSiteFilter] = useState('');
    const [statusFilter, setStatusFilter] = useState('');

    useEffect(() => {
        onGetSensors(siteFilter, statusFilter, 'GET_SENSORS');
    }, [onGetSensors, siteFilter, statusFilter]);
